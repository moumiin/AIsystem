...
KCISA_REFERENCE_PATH = os.path.join(BASE_DIR, "kcisa_reference_data.json")
KCISA_KEYPOINT_PATH = os.path.join(BASE_DIR, "kcisa_keypoint_sequences.json")
...
async def sign_search(req: SearchRequest):
    ...
    # ② 로컬 KCISA 키포인트 시범 데이터
    kcisa_keypoint = _read_json(KCISA_KEYPOINT_PATH, {}).get(q)
    if kcisa_keypoint:
        sequence = kcisa_keypoint.get("sequence") or []
        if sequence:
            print(f"[KCISA-KP] '{q}' → 로컬 키포인트 시범 반환")
            return {
                "name": q,
                "source": "kcisa_keypoint",
                "type": "sequence",
                "data_format": "openpose150",
                "hands": kcisa_keypoint.get("hands", 2 if q != "나" else 1),
                "sequence": sequence,
                "landmarks": sequence[len(sequence) // 2],
                "description": kcisa_keypoint.get("description", f"{q} KCISA 로컬 키포인트 시범"),
                "hint": "얼굴, 팔, 손이 보이는 시범 애니메이션을 따라해보세요",
                "steps": [],
            }
    ...
    # ④ KCISA 영상 데이터 fallback
    kcisa = _read_json(KCISA_REFERENCE_PATH, {}).get(q)
    if kcisa:
        print(f"[KCISA] '{q}' → 영상 시범 반환")
        return {
            "name": q,
            "source": "kcisa",
            "type": "video",
            "videoUrl": kcisa.get("videoUrl"),
            "thumbnailUrl": kcisa.get("thumbnailUrl"),
            "sourceUrl": kcisa.get("sourceUrl"),
            "description": kcisa.get("description", f"{q} KCISA 수어 영상"),
            "hint": "KCISA 수어 영상 시범을 보고 얼굴, 팔, 손 동작을 함께 따라해보세요",
            "steps": [],
            "hands": 2 if q != "나" else 1,
        }
...
def get_all_jamos():
    ...
    for key, entry in JAMO_DB.items():
        result.append({
            ...
            "category": "jamo",
            "emoji": key,
            "pose": entry.get("demo_landmarks", entry["landmarks"]),
            "referenceVariants": entry.get("reference_variants", [entry["landmarks"]]),
            "datasetPose": entry.get("dataset_landmarks"),
