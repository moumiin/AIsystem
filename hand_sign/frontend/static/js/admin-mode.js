class AdminMode {
  constructor(appGetter) {
    ...
    this.aiSampleBtn = document.getElementById('admin-ai-sample-btn');
    this.aiTrainBtn = document.getElementById('admin-ai-train-btn');
    ...
    this.aiSampleBtn?.addEventListener('click', () => this.saveAiSample());
    this.aiTrainBtn?.addEventListener('click', () => this.trainAiModel());
  ...
  async saveAiSample() {
    const app = this.getApp();
    const sign = this.sign;
    const label = sign?.emoji || sign?.id?.replace(/^jamo_/, '');

    if (!app?.tracker?.isRunning) {
      this.setStatus('카메라가 꺼져 있습니다. 먼저 카메라를 시작해주세요.', 'error');
      return;
    }
    if (!sign || sign.category !== 'jamo' || !label) {
      this.setStatus('AI 샘플은 지문자를 선택했을 때만 저장할 수 있습니다.', 'error');
      return;
    }
    if (!Array.isArray(app.lastNormalizedPose) || app.lastNormalizedPose.length < 21) {
      this.setStatus('아직 손 자세가 인식되지 않았습니다. 손을 카메라 안에 보여주세요.', 'error');
      return;
    }

    try {
      const res = await fetch('/api/jamo-ai/sample', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          label,
          pose: this.roundPose(app.lastNormalizedPose),
        }),
      });
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
      this.setStatus(`${label} AI 샘플을 저장했습니다. 저장 후 지문자 AI 학습을 눌러주세요.`, 'ok');
      return data;
    } catch (err) {
      this.setStatus(`AI 샘플 저장 실패: ${err.message || err}`, 'error');
      return null;
    }
  }
  ...
  async trainAiModel() {
    const app = this.getApp();
    try {
      this.setStatus('지문자 AI 모델을 학습하는 중입니다...', '');
      const res = await fetch('/api/jamo-ai/train', { method: 'POST' });
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
      if (app?._loadJamoAiModel) {
        await app._loadJamoAiModel();
      }
      const model = data.model || {};
      this.setStatus(`AI 학습 완료: 원본 ${model.sample_count || 0}개 → 증강 ${model.augmented_sample_count || 0}개, 사용자 샘플 ${model.user_sample_count || 0}개`, 'ok');
      return data;
    } catch (err) {
      this.setStatus(`AI 학습 실패: ${err.message || err}`, 'error');
      return null;
    }
  }
