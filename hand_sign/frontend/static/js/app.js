...
class SignLanguageApp {
  constructor() {
    ...
    this._staticResultModalShown = false;
  ...
  _renderGrid(category) {
    const grid = document.getElementById('sign-grid');
    const signs = SIGNS.filter(s => s.category === category && !s.hidden);
  ...
  _syncDemoHandSlots(sign) {
    ...
    if (this._isFingerSpellingSign(sign) && Array.isArray(sign.referenceVariants)) {
      const variants = sign.referenceVariants
        .filter(pose => Array.isArray(pose) && pose.length >= 21)
        .map(pose => pose.slice(0, 21));
      if (variants.length > 0) {
        sign._demoHandSlots = sign._demoHandSlots.map(slot => ({
          ...slot,
  ...
  _matchHandsToRefs(hands, refs, strictShape = false) {
    ...
    const scoreAgainstSlot = (landmarks, handedness, slotEntry) => {
      ...
      const rawRefs = Array.isArray(slotEntry.referenceVariants) && slotEntry.referenceVariants.length > 0
        ? [...slotEntry.referenceVariants, slotEntry.landmarks]
        : [slotEntry.landmarks];
      const refs = [];
      rawRefs.forEach(ref => {
        if (!Array.isArray(ref) || ref.length < 21) return;
        if (refs.some(item => JSON.stringify(item.slice(0, 21)) === JSON.stringify(ref.slice(0, 21)))) return;
        refs.push(ref);
      });
      ...
      refs.forEach((ref, index) => {
        const breakdown = computeScoreBreakdown(norm, ref, {
          strict: true,
          tryRotation: strictShape,
        });
        if (!breakdown) return;
        const candidate = { breakdown, ref, variantIndex: index };
        if (!best || breakdown.score > best.breakdown.score) best = candidate;
      });
      if (!best) return null;

      const primary = refs[0];
      const primaryBreakdown = computeScoreBreakdown(norm, primary, {
        strict: true,
        tryRotation: strictShape,
      });
      if (primaryBreakdown && best.variantIndex > 0 && primaryBreakdown.score + 6 >= best.breakdown.score) {
        best = { breakdown: primaryBreakdown, ref: primary, variantIndex: 0 };
      }
      ...
      return {
        ...
        norm,
        scoreRef: best.ref,
        scoreVariant: best.variantIndex,
        slot: slotEntry.slot,
        handedness,
        perFinger: best.breakdown.fingerScores,
  ...
  _onHandResult(hands) {
    ...
    if (!hands.length) {
      if (isStaticSign) {
        const result = this._lastGestureResult;
        if (result && !this._staticResultModalShown && (result.score > 0 || this.bestScore > 0)) {
          this._staticResultModalShown = true;
          this._setStatus(`마지막 평가: ${result.score}점`);
          this._showResultModal(result, Boolean(result.ok));
          return;
        }
        this._resetGestureRecording();
        this._noHandStreak = 0;
    ...
    this._noHandStreak = 0;
    if (isStaticSign) {
      if (this._staticResultModalShown) {
        this._lastGestureResult = null;
        this.bestScore = 0;
        this.scoreBuffer = [];
        this.successFrames = 0;
    ...
    if (isStaticSign) {
      ...
      const staticFeedback = [
        ...
        '손을 치우면 이번 손모양 점수 팝업이 표시됩니다.',
      ].join('<br>');

      this._lastGestureResult = {
        score: smoothScore,
        ok: passed,
        message: passed ? '정적 손모양이 기준과 잘 맞았어요' : '손모양을 조금 더 맞춰보세요',
        feedback: staticFeedback,
        poseScore: smoothScore,
        trajectoryScore: 0,
        movementScore: 0,
        handOkRatio: idealRequiredHands === 2 ? selected.length / 2 : 1,
        perFinger: bestPerFinger,
        format,
        frameCount: this.scoreBuffer.length,
        durationMs: Math.round(this.scoreBuffer.length * 33),
      };
      this._setStatus(passed ? `✅ 정답에 가까워요: ${smoothScore}점` : `실시간 점수: ${smoothScore}점`);
      this._setLiveFeedback(staticFeedback);
  ...
  _goNextSign() {
    const visibleSigns = SIGNS.filter(s => !s.hidden);
    const idx  = visibleSigns.findIndex(s => s.id === this.currentSign?.id);
    const next = visibleSigns[(idx + 1) % visibleSigns.length];
  ...
  playOpenPose(signData) {
    ...
    if (signData?.videoUrl && !signData.pose && !signData.sequence) {
      this.playReferenceVideo(signData);
      return;
    }
...
drawOpenPoseFrame(feature){
  ...
  const transform = this.buildDemoFrameTransform(feature, width, height);

  this.drawBodySkeleton(ctx, pose, width, height, transform);
  this.drawHead(ctx, pose, width, height, transform);
  this.drawHand(ctx, left, width, height, '#22c55e', transform);
  this.drawHand(ctx, right, width, height, '#f97316', transform);
  this.drawPoints(ctx, left, width, height, '#22c55e', 2.2, transform);
  this.drawPoints(ctx, right, width, height, '#f97316', 2.2, transform);
}
